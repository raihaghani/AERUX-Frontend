...
"""
Test-set inference with per-sample heatmap overlays and aggregate metrics for DenseNet models.
...
Usage:
    python infer_test_set_Densenet.py --fusion_model path/to/best_fusion_model.pth
"""
...
from train_multitask_DenseNet import MultiTaskDenseNet
from train_fusion_Densenet import SimpleFusionModel


def load_fusion_model_densenet(checkpoint_path, device):
    """Load the trained DenseNet fusion model"""
    print(f"Loading DenseNet fusion model from: {checkpoint_path}")
    ...
    if 'config' in checkpoint and 'checkpoint_paths' in checkpoint['config']:
        ...
        for modality in ['CTA', 'MRA', 'MRI']:
            pattern = f"E:/Education/Aerux_Final/outputs/multitask_DenseNet_{modality}_*/best_model_{modality}.pth"
    ...
    for modality, model_path in modality_checkpoints.items():
        print(f"Loading {modality} DenseNet base model from {model_path}...")
    ...
    # Create fusion model
    fusion_strategy = checkpoint.get('fusion_strategy', 'attention')
    fusion_model = SimpleFusionModel(modality_models, fusion_strategy=fusion_strategy)
    fusion_model.load_state_dict(checkpoint['model_state_dict'])
    fusion_model = fusion_model.to(device)
    fusion_model.eval()
    
    print("[OK] DenseNet Fusion model loaded successfully\n")
    return fusion_model
...
def main():
    parser = argparse.ArgumentParser(description="Run DenseNet fusion inference on the full test set")
    ...
    if args.out_dir is None:
        ...
        args.out_dir = Path(config.data.output_root) / f"test_set_inference_densenet_{ts}"
    ...
    # Load DenseNet fusion model
    model = load_fusion_model_densenet(str(args.fusion_model), device)
    ...
    # Print summary
    print("\n" + "=" * 80)
    print("TEST SET INFERENCE RESULTS (DENSENET)")
    print("=" * 80)
