...
from train_multitask_DenseNet import MultiTaskDenseNet, MultiTaskLoss, compute_weighted_auc, LOCATION_LABELS
    ...
    def forward(self, inputs: Dict[str, Optional[torch.Tensor]], modality_indices: Dict[str, list]):
        ...
        for modality in self.modalities:
            if modality in inputs and inputs[modality] is not None:
                ...
                # Extract features from frozen encoder (MultiTaskDenseNet architecture)
                model = self.modality_models[modality]
                with torch.no_grad():
                    # Forward through DenseNet121 encoder layers to get 1024-dim features
                    x = model.relu0(model.norm0(model.conv0(modality_tensor)))  # 64ch, 128x128
                    x = model.pool0(x)                                           # 64ch, 64x64
                    x = model.denseblock1(x)                                     # 256ch, 64x64
                    x = model.denseblock2(model.transition1(x))                  # 512ch, 32x32
                    x = model.denseblock3(model.transition2(x))                  # 1024ch, 16x16
                    x = model.denseblock4(model.transition3(x))                  # 1024ch, 8x8
                    x = F.relu(model.norm5(x))
                    # Global average pooling
                    features = model.avgpool(x)  # [B, 1024, 1, 1]
...
def train_fusion_model(config, checkpoint_paths, num_epochs=30, batch_size=4, learning_rate=1e-5):
    ...
    output_dir = os.path.join(config.data.output_root, f"fusion_DenseNet_{timestamp}_most_final_experiment")
    ...
    # Internal fusion test split (legacy only). With global split, use infer_test_set.py on global_test_series.csv.
    test_metrics = None
    ...
    if test_metrics is not None:
        # 2–4. Internal fusion test-set plots (legacy split only)
